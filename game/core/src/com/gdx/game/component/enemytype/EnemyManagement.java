        ...
        switch (gameStage) {
            case 1:
                for (int time = 0 ; time <= GAME_TIME_END; time += 1){
                    if (time >= 0 && time < 40)spawnEnemy(delta, time, TypeEnemy.MONSTER_1, j, i);
                    if (time % 3 == 0 && time < 30 && time > 15)spawnEnemy(delta, time, TypeEnemy.MONSTER_4, j, i);
                    if (time % 5 == 0 && time < 50 && time > 15)spawnEnemy(delta, time, TypeEnemy.MONSTER_6, j, i);
                }
                break;
